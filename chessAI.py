...
def label(board, top, clicks):
    for r in range(0, 8):
        for c in range(0, 8):
            ...
            if(pieceName != ""):
                imageName = "images/" + (("".join(pieceName.split())).lower() + ".gif")
                #imageName = os.path.abspath("Documents\\Python Scripts\\chessAIC\\images\\" + ("".join(pieceName.split())).lower() + ".gif") #for testing in IEP
